...
001	sgh-1 26-11-2019 [Nishit Khakhi]	Added Initial Version to Delete Company
*******************************************************************/
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Company_Delete' AS '@procName'             
    ...
    BEGIN TRY            
		...
		IF NOT EXISTS (SELECT TOP 1 1 FROM [Company] (NOLOCK) WHERE [guid] = @companyguid AND [isdeleted] = 0)
		BEGIN
			SET @output = -3
