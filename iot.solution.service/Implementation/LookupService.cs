...
namespace iot.solution.service.Data
{
    public class LookupService : ILookupService
    {
        ...
        public List<Entity.LookupItem> SubEntityLookup(Guid entityId, string type)
        {
            List<Entity.LookupItem> result = new List<Entity.LookupItem>();
            ...
            {
                result = (from t in _entityRepository.GetAll()
                          where t.CompanyGuid.Equals(SolutionConfiguration.CompanyId) && t.ParentEntityGuid.Equals(entityId) && (string.IsNullOrWhiteSpace(type) || t.Type.Equals(type))  && !t.IsDeleted
                          select new Entity.LookupItem()
                          {
                              Text = t.Name,
                              Value = t.Guid.ToString()
        ...
        public Entity.BaseResponse<List<Entity.AttributeItem>> DeviceAttributeLookup(Guid deviceId)
        {
            ...
            try
            {
                _logger.Information(Constants.ACTION_ENTRY, "Device_LatestAttributeValue_Get.Get");
